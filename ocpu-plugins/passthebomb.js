...
class PassTheBomb {
	...
	updateJoins() {
		...
		if (this.players.size > 0) {
			msg += `<center><strong>${this.players.size}</strong> ${(this.players.size === 1 ? `user has` : `users have`)} joined: ${Array.from(this.players).map(player => OCPU.nameColor(player[1].name).join(", "))}</center>`;
		}
	...
	getMsg() {
		...
		msg += `Players: ${this.getSurvivors().map(player => OCPU.nameColor(player[1].name).join(", "))}<br />`;
	...
	nextRound() {
		...
		this.release = setTimeout(() => {
			...
			this.room.add(`|uhtmlchange|${this.getMsg()}<br /><strong style="font-size: 10pt;">The bomb has been passed to </strong>${OCPU.nameColor(player, true)}</div>`).update();
			this.canPass = true;
			this.resetTimer();
	...
	pass(user, target, self) {
		...
		this.room.add(`|html|${OCPU.nameColor(user.name, true)} passed the bomb to ${OCPU.nameColor(targetId, true)}</strong>!`);
	...
	resetTimer() {
		this.timer = setTimeout(() => {
			let player = this.players.get(this.holder).name;
			this.room.add(`|html|The bomb exploded and killed ${OCPU.nameColor(player, true)}`).update();
	...
	removeUser(userid, left) {
		...
		this.room.add(`|html|${OCPU.nameColor(this.players.get(userid).name, true)} has ${(left ? `left` : `been disqualified from`)} the game.`);
		this.players.delete(userid);
		this.madeMove = true;
		if (this.checkWinner()) {
			this.getWinner();
			...
			this.setBomb();
			let player = this.players.get(this.holder).name;
			this.room.add(`|html|The bomb has been passed to ${OCPU.nameColor(player, true)}!`).update();
		}
	}
	...
	getWinner() {
		...
		let msg = `|html|<div class="infobox"><center>The winner of this game of Pass The Bomb is ${OCPU.nameColor(winner, true)}! Congratulations!</center>`;
		if (this.room.isOfficial) {
			OCPU.ExpControl.addExp(winner, this.room, 5);
			msg += `${OCPU.nameColor(winner, true)} has also won 5 EXP for winning this game of Pass The Bomb.`;
		}
		...
		if (user) {
			let msg = `<div class="infobox"><center>This game of Pass The Bomb has been forcibly ended by ${OCPU.nameColor(user.name, true)}.</center></div>`;
			if (!this.madeMove) {
...
exports.commands = {
	...
	passthebomb: {
		...
		proceed: function (target, room, user) {
			...
			room.addRaw(`(${OCPU.nameColor(user.name, true, true)} forcibly started the game of Pass The Bomb.)`);
