...
class RPSGame {
	...
	sendGameInformation(player, opponent) {
		let pmPost = `/html <div class="broadcast-green"><center>You have been matched up with ${OCPU.nameColor(opponent.name, true)}<br />`;
	...
	onEnd(inactivity) {
		...
		if (gameResult === "pp") {
			// tie
			this.p1.send(`|pm|~Rock/Paper/Scissors${this.gameType !== "ladderRPSLS" ? `` : `/Lizard/Spock`} Host|${this.p1.userid}|/html The game with ${OCPU.nameColor(this.p2.name, true)} was a tie! ${OCPU.nameColor(this.p2.name, true)} has chose ${choiceNames[this.p2choice]}.`);
			this.p2.send(`|pm|~Rock/Paper/Scissors${this.gameType !== "ladderRPSLS" ? `` : `/Lizard/Spock`} Host|${this.p2.userid}|/html The game with ${OCPU.nameColor(this.p1.name, true)} was a tie! ${OCPU.nameColor(this.p1.name, true)} has chose ${choiceNames[this.p1choice]}.`);
	...
	parseWin(winner, loser, inactivity) {
		winner.send(`|pm|~Rock/Paper/Scissors${this.gameType !== "ladderRPSLS" ? `` : `/Lizard/Spock`} Host|${winner.userid}|/html You have won the game against ${OCPU.nameColor(loser.name, true)}! ${(!inactivity ? `${OCPU.nameColor(loser.name, true)} has chose ${choiceNames[(winner.userid === this.p1.userid ? this.p2choice : this.p1choice)]}.` : ``)}`);
		loser.send(`|pm|~Rock/Paper/Scissors${this.gameType !== "ladderRPSLS" ? `` : `/Lizard/Spock`} Host|${loser.userid}|/html You have lost the game against ${OCPU.nameColor(winner.name, true)}! ${(!inactivity ? `${OCPU.nameColor(winner.name, true)} has chose ${choiceNames[(loser.userid === this.p1.userid ? this.p2choice : this.p1choice)]}.` : ``)}`);
		if (this.gameType === "bucks") {
			// set but bucks
			...
			winner.send(`|pm|~Rock/Paper/Scissors${this.gameType !== "ladderRPSLS" ? `` : `/Lizard/Spock`} Host|${winner.userid}|/html ${OCPU.nameColor(winner.name, true)}: ${winnerPoints} --> ${winnerFinalPoints}<br />${OCPU.nameColor(loser.name, true)}: ${loserPoints} --> ${loserFinalPoints}`);
			loser.send(`|pm|~Rock/Paper/Scissors${this.gameType !== "ladderRPSLS" ? `` : `/Lizard/Spock`} Host|${loser.userid}|/html ${OCPU.nameColor(winner.name, true)}: ${winnerPoints} --> ${winnerFinalPoints}<br />${OCPU.nameColor(loser.name, true)}: ${loserPoints} --> ${loserFinalPoints}`);
		}
	}
}
...
exports.commands = {
	...
	rps: {
		...
		rank: function (target, room, user) {
			...
			this.sendReplyBox(`<strong>Rank - ${OCPU.nameColor(target, true)}: ${userRank}</strong>`);
		},
