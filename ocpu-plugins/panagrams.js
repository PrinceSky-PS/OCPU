...
if (!OCPU.pGames) OCPU.pGames = {};
let pGames = OCPU.pGames;
...
class Panagram {
	...
	guess(user, guess) {
		if (guess.species === this.answer.species) {
			this.room.add(`|html|${OCPU.nameColor(user.name, true)} guessed <strong>${guess.species}</strong>, which was the correct answer! This user has also won 1 ${currencyName}!`);
			Economy.writeMoney(user.userid, 1);
			this.end();
		} else {
			this.room.add(`|html|${OCPU.nameColor(user.name, true)} guessed <strong>${guess.species}</strong>, but was not the correct answer...`);
...
exports.commands = {
	...
	panagramend: function (target, room, user, connection, cmd) {
		...
		if (skipCmd) room.add(`|html|The current session of panagram has been ended by ${OCPU.nameColor(user.name, true)}. The answer was <strong>${pGames[room.id].answer.species}</strong>.`);
