...
function display(result, user, slotOne, slotTwo, slotThree) {
	...
	if (!result) {
		display += `Aww... bad luck, ${OCPU.nameColor(user, true)}. Better luck next time!`;
	} else {
		display += `Congratulations, ${OCPU.nameColor(user, true)}. You have won ${slots[slotOne]} ${currencyPlural}!`;
	}
