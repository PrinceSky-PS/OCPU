...
exports.commands = {
	...
	sc: {
		...
		set: function (target, room, user) {
			...
			this.sendReply(`|raw|You have given ${OCPU.nameColor(target[0], true)} an symbol color.`);
			Monitor.log(`${target[0]} has received an symbol color from ${user.name}.`);
			this.privateModAction(`|raw|(${OCPU.nameColor(target[0], true)} has received a symbol color: <font color="${target[1]}">${target[1]}</font> from ${user.name}.)`);
			if (Users(target[0]) && Users(target[0]).connected) Users(target[0]).popup(`|html|${OCPU.nameColor(user.name, true)} has set your symbol color to: <font color="${target[1]}">${target[1]}</font>.<br /><center>Refresh, If you don't see it.</center>`);
		...
		delete: function (target, room, user) {
			...
			if (Users(target) && Users(target).connected) Users(target).popup(`|html|${OCPU.nameColor(user.name, true)} has removed your symbol color.`);
		},
