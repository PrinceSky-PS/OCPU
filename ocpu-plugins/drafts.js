...
class Draft {
	...
	addTeam(teamname, manager, self) {
		...
		this.room.add(`|html|<div style="${greencss}">The <strong>${teamname}</strong> are now apart of the draft and is managed by ${OCPU.nameColor(manager, true)}</div>`);
	...
	Nom(pk, user, self) {
		...
		if (this.order.length === this.order.indexOf(this.turn) + 1) {
			if (this.teams[this.turn].draftpicks.length === this.maxMons) {
				this.room.add(`|html|<div style="${greencss}">${OCPU.nameColor(user.name, true)} has drafted the Pokemon: <strong>${pk}!</strong></div>`);
				...
				this.room.add(`|html|<div style="${greencss}">${OCPU.nameColor(user.name, true)} has drafted the Pokemon: <strong>${pk}.</strong><br />It is now <strong>${this.turn}'s</strong> turn.</div>`);
				...
				this.turn = this.order[0];
				this.room.add(`|html|<div style="${greencss}">${OCPU.nameColor(user.name, true)} has drafted the Pokemon: <strong>${pk}.</strong><br />It is now <strong>${this.turn}'s turn</strong>.</div>`);
				this.room.add(`|html|<div style="${greencss}"><strong>${this.turn}</strong> currently has: ${this.iconize(this.teams[this.turn].draftpicks)}</div>`);
				this.log(`${user} has drafted ${pk}.`);
			}
		} else {
			this.turn = this.order[this.order.indexOf(this.turn) + 1];
			this.room.add(`|html|<div style="${greencss}">${OCPU.nameColor(user.name, true)} has drafted the Pokemon: <strong>${pk}!</strong><br />It is now <strong>${this.turn}</strong>'s turn.</div>`);
			this.room.add(`|html|<div style="${greencss}"><strong>${this.turn}</strong> currently has: ${this.iconize(this.teams[this.turn].draftpicks)}</div>`);
			this.log(`${user} has drafted ${pk}.`);
		}
	}
