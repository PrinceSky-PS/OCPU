...
function alertDevs(message) {
	OCPU.devPM("~Developer Chat", `New task: ${message}`);
...
exports.commands = {
	...
	tasks: {
		...
		add: function (target, room, user) {
			if (!OCPU.isDev(user.userid) && !this.can("bypassall")) return false;
			...
			alertDevs(`${OCPU.nameColor(user.name, true, true)} has filed an issue.<br />Issue: ${issue}.<br />Description: ${description}.<br />Priority: ${priority}.`);
			return this.sendReply(`The task "${issue}" has been added to the OCPU Task List.`);
		},
		...
		list: function (target, room, user) {
			...
			if (Object.keys(taskList.issues).length < 1) return this.errorReply(`There are currently no issues on OCPU.`);
			let display = `<center><h1>OCPU's Tasks List:</h1><table border="1" cellspacing ="0" cellpadding="4"><tr style="font-weight: bold"><td>Employer</td><td>Issue Title</td><td>Issue Description</td><td>Issue Priority</td></tr>`;
			...
			sortedIssues.forEach(task => {
				display += `<tr>`;
				display += `<td style="border: 2px solid #000000; width: 20%; text-align: center"><button class="button" name="parseCommand" value="/user ${taskList.issues[task].employer}">${OCPU.nameColor(taskList.issues[task].employer, true, true)}</button></td>`;
