...
class SentenceGame {
	...
	addWord(user, word, self) {
		...
		this.room.addRaw(`${OCPU.nameColor(user.name, true, true)} has added "${word}" to the sentence.<br />Sentence: ${this.sentence.join(" ")}.<br />${wordsLeft === 0 ? `` : `<small>There ${wordsLeft === 1 ? `is` : `are`} ${wordsLeft} word${wordsLeft === 1 ? `` : `s`} remaining in this Sentence Game.</small>`}`);
	...
	endGame(forced) {
		...
		if (forced) this.room.addRaw(`${OCPU.nameColor(forced, true, true)} has ended the Sentence Game in this room forcefully.`);
...
exports.commands = {
	...
	sentence: {
		...
		show: function (target, room, user) {
			...
			return this.sendReplyBox(`<strong>Current Sentence:</strong> ${room.sentence.sentence.join(" ")}.<br /><strong>Progress:</strong> ${room.sentence.sentence.length}/${room.sentence.wordCount} words.<br /><strong>Last User:</strong> ${OCPU.nameColor(room.sentence.lastUser, true, true)}.`);
		},
