...
class Dice {
	...
	join(user, self) {
		...
		Economy.readMoney(user.userid, money => {
			...
			this.room.add(`|uhtmlchange|${this.room.diceCount}|${this.startMessage}<center>${OCPU.nameColor(user.name, true)} has joined the game!</center></div>`).update();
	...
	play() {
		...
		Economy.readMoney(p1.userid, money1 => {
			Economy.readMoney(p2.userid, money2 => {
				if (money1 < this.bet || money2 < this.bet) {
					...
					this.players.splice(this.players.indexOf(user), 1);
					this.room.add(`|uhtmlchange|${this.room.diceCount}|${this.startMessage}<center>${this.players.map(user => OCPU.nameColor(user.name, true))} has joined the game!</center>`).update();
					return;
				}
				let players = this.players.map(user => OCPU.nameColor(user.name, true)).join(" and ");
				...
				setTimeout(() => {
					...
					msg += `<img src = "${diceImg(roll1)}" align = "left" title = "${OCPU.nameColor(p1.name, true)}'s roll">`;
					msg += `<img src = "${diceImg(roll2)}" align = "right" title = "${OCPU.nameColor(p2.name, true)}'s roll"><br />`;
					msg += `${OCPU.nameColor(p1.name, true)} rolled ${(roll1 + 1)}!<br />`;
					msg += `${OCPU.nameColor(p2.name, true)} rolled ${(roll2 + 1)}!<br />`;
					msg += `${OCPU.nameColor(winner.name, true)} has won <strong style="color: green">${(this.bet - taxedAmt)}</strong> ${buck}!<br />`;
					msg += `Better luck next time, ${OCPU.nameColor(loser.name, true)}!`;
		...
		if (user) this.room.add(`|uhtmlchange|${this.room.diceCount}|<div class="infobox">(This game of dice has been forcibly ended by ${OCPU.nameColor(user.name, true, true)}.)</div>`).update();
...
exports.commands = {
	...
	dicegame: function (target, room, user) {
		...
		if (target.includes(".") || amount < 1 || amount > 5000) return this.errorReply(`The number of ${currencyPlural} must be between 1 and 5,000 and cannot contain a decimal.`);
		Economy.readMoney(user.userid, money => {
			if (money < amount) return this.errorReply(`You don't have ${amount} ${money === 1 ? currencyName : currencyPlural}.`);
	...
	dicegamehelp: [
		`/startdice or /dicegame [amount] - Starts a game of dice in the room for a given number of ${global.currencyPlural}, 1 ${global.currencyName} by default.
		/joindice - Joins the game of dice. You cannot use this command if you don't have the number of ${global.currencyPlural} the game is for.
